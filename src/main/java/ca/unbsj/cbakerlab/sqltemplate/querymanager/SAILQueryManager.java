...
public class SAILQueryManager {
     ...
     */
    public String generateSQLTemplate(String[] args, String schematicAnswers) throws Exception {

    	Set<String> setOfSQLQueries = new HashSet<String>();
		String VPAutoGeneratedSQLQuery = "";
	...
	while ((line = input.readLine()) != null)
		...
		if (line.trim().startsWith("<generic_answer>"))
			...
			System.out.println("generic answers : "+genAnswer);
			...
			for (SQLQuery q : SQLQueries)
				...
				System.out.println(q.queryVarMap());
				
				queryCounter++;
				// assign if there is a query
				//if( !q.queryText().equals("") ){
					
				// store each unique query in a set
				if(!q.queryText().isEmpty())
					setOfSQLQueries.add(q.queryText());
				//VPAutoGeneratedSQLQuery = q.queryText();
				//System.out.println("Query="+queryCounter);
				//System.out.println(VPAutoGeneratedSQLQuery);
				...
				//}
                // get a smaller size query if there is any
                //for(String query : generatedQueryList){
                	//if(query.length() < VPAutoGeneratedSQLQuery.length())
                	//VPAutoGeneratedSQLQuery = query;
               // }
	...
	String tempQuery = "";
	Iterator setIterator = setOfSQLQueries.iterator();
	//GET THE FIRST SQL QUERY
	if(setIterator.hasNext())
		VPAutoGeneratedSQLQuery = setIterator.next().toString();
	
	//assign a new query only if it is smaller (assuming less joining of tables) than the current query
	while (setIterator.hasNext()) {
		tempQuery = setIterator.next().toString(); 
		if(tempQuery.length() < VPAutoGeneratedSQLQuery.length())
			VPAutoGeneratedSQLQuery = tempQuery;
	}
		
		return VPAutoGeneratedSQLQuery;
    } // void generateSQLTemplate(String[] args)
