...
/**
 * This class executes a single example query and reports the results. Use this
 * for tracking down problems in specific queries, since Eclipse apparently runs
 * all JUnit tests even if you ask it to just run one of them.
 *
...
//			"}";
        ...
        // does not work
        query = "PREFIX servOnt: <http://cbakerlab.unbsj.ca:8080/haitohdemo/haitoh-sadi-service-ontology.owl#>\n"
                + "PREFIX HAIOnt: <http://cbakerlab.unbsj.ca:8080/haitohdemo/HAI.owl#>\n"
                + "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n"
                + "\n"
                + "SELECT ?patient ?diagnosis_code\n"
                + "FROM <http://cbakerlab.unbsj.ca:8080/haitohdemo/getDiagnosisIDByPatientID_input.rdf>\n"
                + "WHERE{\n"
                + "	?patient a HAIOnt:Patient .\n"
                + "	?patient HAIOnt:has_diagnosis ?diagnosis .\n"
                + "	?diagnosis a HAIOnt:Diagnosis .\n"
                + "	?diagnosis HAIOnt:has_diagnosis_code ?diagnosis_code .	\n"
                + "}";
        // WORKS for the service getDiagnosisIDByPatienetID
        query = "PREFIX HAIOnt: <http://cbakerlab.unbsj.ca:8080/haitohdemo/HAI.owl#>\n"
                + "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n"
                + "\n"
